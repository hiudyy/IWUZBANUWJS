...
const getTikTokBuffer = async (url) => {
  if (!url) {
    return {
      error: 'Url não proporcionada ',
      message: 'Por favor, digite um link do tiktok valido, exemplo: ' +
    ...
    try {
      ...
      try {
        ...
        try {
          ...
          return {
            error: 'Erro interno nas apis',
            message: 'Não se encontra resultados, tente novamente mais tarde.'
          };
        }
      }
    }
...
async function tiktokdlF(url) {
  if (!/tiktok/.test(url)) return 'Link incorreto';
